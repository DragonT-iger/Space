...
<div class="bookingCalendar" id="bookingCalendar">
	<div class="section_inner">
		<div class="time_title">
			<span class="option_label">날짜선택::</span> <span id="check_month">${today_info.search_month}</span>월
						...
						<td class="normal_day">
							...
							<div id="sch_date">
								...
								<c:if test="${scheduleList.rtstartdate eq dList.YMD}">
									${schedule_data_arr_status.count}
								</c:if>
								</c:forEach> --%>
								<c:forEach var="cnt" items="${sch_list_count}">
									<c:if test="${cnt.rtstartdate eq dList.YMD}">
										${cnt.rcount}건
									</c:if>
								</c:forEach>
							</div>	
